...
const StoreInfo = ({store}) => {
    const rawContact = store?.contact?.trim() || ""
    const telContact = rawContact.replace(/[^\d+]/g, "")
    const displayCourseId = store?.courseId || store?.course || "N/A"

    return (
        <div className="flex-1 space-y-2 text-sm">
            <Image width={100} height={100} src={ikImage(store.logo, { width: 160 })} alt={store.name} className="max-w-20 max-h-20 object-contain shadow rounded-full max-sm:mx-auto" />
            ...
            {telContact ? (
                ...
                >
                    ...
                    <span className="inline-flex items-center rounded bg-slate-800 px-2 py-0.5 text-[11px] text-white">Call</span>
                </a>
            ) : (
                <p className="flex items-center gap-2"><Phone size={16} /> {store.contact || "N/A"}</p>
            )}
            <p className="flex items-center gap-2"><Mail size={16} />  {store.email}</p>
            <p className="text-slate-800 font-bold">Course ID: {displayCourseId}</p>
